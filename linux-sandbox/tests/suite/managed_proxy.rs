...
async fn run_linux_sandbox_direct(
    ...
    let fs_policy_json = match serde_json::to_string(fs_policy) {
        Ok(json) => json,
        Err(err) => panic!("fs policy should serialize: {err}"),
    };
    let net_policy_json = match serde_json::to_string(&net_policy) {
        Ok(json) => json,
        Err(err) => panic!("net policy should serialize: {err}"),
    };
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    .await;
    assert!(!direct_egress_output.status.success());
}
