...
from agent_sec_cli.sandbox.bash_ast import BashAstAnalyzer
...
class CommandClassifier:
    ...
    def classify(self, command: str) -> Dict[str, Any]:
        ...
        ast_analysis = self.bash_ast.analyze(command)
        ast_decision, ast_reason = ast_analysis.best_decision()

        if ast_decision == "destructive":
            return {
                "decision": "destructive",
                "reason": ast_reason,
                "command": command,
                "additional_permissions": None,
                "ast": ast_analysis.to_dict(),
            }
        if ast_decision == "dangerous":
            return {
                "decision": "dangerous",
                "reason": ast_reason,
                "command": command,
