...
class SandboxPolicyBuilder:
    ...
    def _build_argv(
        ...
        argv.extend(SandboxPolicyBuilder._command_argv(command))
    ...
    def _build_shell_command(
        ...
        return (
            ...
            f" -- {SandboxPolicyBuilder._command_preview(command)}"
        )
    ...
    def _command_argv(command: str) -> List[str]:
        if SandboxPolicyBuilder._requires_shell_wrapper(command):
    ...
    def _command_preview(command: str) -> str:
        argv = SandboxPolicyBuilder._command_argv(command)
        return " ".join(shlex.quote(part) for part in argv)

    @staticmethod
    def _requires_shell_wrapper(command: str) -> bool:
        try:
        ...
        if any(op in command for op in SandboxPolicyBuilder._COMPOUND_OPERATORS):
            return True

        if any(ch in command for ch in SandboxPolicyBuilder._SHELL_METACHARS):
            return True
